...
$insert = @"
    ...
    async exportDashboard() {
        try {
            ...
            // Use current report period for export
...
"@
# Insert after loadDashboardStats method ends (find the closing brace and insert after)
$content = $content -replace '(async loadDashboardStats\(\) \{[^}]+\s+this\.loadRecentSales\(this\._recentSalesPeriod\);[^}]+\s+this\._periodLabel\(this\._recentSalesPeriod\)\;\s+\}\s*\})', "`$1$insert"
