...
"@
# Insert after refreshOverviewMetricsSoon method (find it by the setTimeout pattern)
$content = $content -replace '(this\.overviewRefreshTimer = setTimeout\(\(\) => \{[^}]+\}, 600\);\s+\}\s*\})', '$1$insert'
