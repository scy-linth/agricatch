...
# Remove the incorrectly placed exportDashboard method
$content = $content -replace '\s+async exportDashboard\(\) \{[^}]+\}\s+', ''
# Insert it before the class closing brace
$insert = @"
...
"@
$content = $content -replace '(}\s*$)', "$insert`$1"
