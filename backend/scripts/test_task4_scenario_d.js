/**
 * Task 4 - Scenario D: Pre-order Converted → Farmer Cancel
 * 
 * Verify that farmer cancelling a converted pre-order restores stock_quantity
...
async function testScenarioD() {
  console.log('=== Task 4 - Scenario D: Pre-order Converted → Farmer Cancel ===\n');
  ...
  try {
    ...
    const farmerResult = await client.query(
      `INSERT INTO users (email, password, role, first_name, last_name, phone, is_verified, username)
       ...
       RETURNING id`,
      ['test_task4_d_farmer@example.com', 'hashed_password', 'farmer', 'Test', 'Farmer D', '9123456759', true, 'task4_farmer_d']
    );
    ...
    const customerResult = await client.query(
      `INSERT INTO users (email, password, role, first_name, last_name, phone, is_verified, username)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING id`,
      ['test_task4_d_customer@example.com', 'hashed_password', 'customer', 'Test', 'Customer D', '9123456758', true, 'task4_customer_d']
    );
    ...
    const productResult = await client.query(
      `INSERT INTO products (farmer_id, name, description, price, stock_quantity, reserved_quantity, 
       ...
       RETURNING id`,
      [farmerId, 'Task 4 Test Product D', 'Test product for Scenario D', 100.00, 
    ...
    // Step 8: Simulate Farmer Cancel (using the same logic as the endpoint)
    // Update order status
    await client.query(
      `UPDATE orders
    ...
    if (errors.length === 0) {
      console.log('\n✅ SCENARIO D PASSED: Pre-order Converted → Farmer Cancel');
    } else {
      console.log('\n❌ SCENARIO D FAILED:');
