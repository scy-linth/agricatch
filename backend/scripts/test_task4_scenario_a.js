/**
 * Task 4 - Scenario A: Pre-order Reserved → Customer Cancel
 * 
 * Verify that cancelling a pre-order reservation releases the reservation
...
async function testScenarioA() {
  console.log('=== Task 4 - Scenario A: Pre-order Reserved → Customer Cancel ===\n');
  ...
  try {
    ...
    const farmerResult = await client.query(
      `INSERT INTO users (email, password, role, first_name, last_name, phone, is_verified, username)
       ...
       RETURNING id`,
      ['test_task4_a_farmer@example.com', 'hashed_password', 'farmer', 'Test', 'Farmer A', '9123456789', true, 'task4_farmer_a']
    );
    ...
    const customerResult = await client.query(
      `INSERT INTO users (email, password, role, first_name, last_name, phone, is_verified, username)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
       RETURNING id`,
      ['test_task4_a_customer@example.com', 'hashed_password', 'customer', 'Test', 'Customer A', '9123456788', true, 'task4_customer_a']
    );
    ...
    const productResult = await client.query(
      `INSERT INTO products (farmer_id, name, description, price, stock_quantity, reserved_quantity, 
       ...
       RETURNING id`,
      [farmerId, 'Task 4 Test Product A', 'Test product for Scenario A', 100.00, 
    ...
    // Step 7: Simulate Customer Cancel (using the same logic as the endpoint)
    // Update order status
    await client.query(
      `UPDATE orders
    ...
    if (errors.length === 0) {
      console.log('\n✅ SCENARIO A PASSED: Pre-order Reserved → Customer Cancel');
    } else {
      console.log('\n❌ SCENARIO A FAILED:');
