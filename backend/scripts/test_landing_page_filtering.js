  ...
  try {
    ...
    const nonApprovedResult = await pool.query(
      `SELECT COUNT(*) as count
       FROM products
       WHERE status != 'approved' AND is_available = true AND is_admin_disabled = false AND stock_quantity > 0`
    );
    ...
    const disabledResult = await pool.query(
      `SELECT COUNT(*) as count
       FROM products
       WHERE is_admin_disabled = true AND is_available = true AND status = 'approved' AND stock_quantity > 0`
    );
    ...
    // Test 3: Check products with stock_quantity = 0 are excluded
    console.log('TEST 3: Verify stock_quantity > 0 filter');
    const outOfStockResult = await pool.query(
      `SELECT COUNT(*) as count
       FROM products
       WHERE stock_quantity = 0 AND is_available = true AND status = 'approved' AND is_admin_disabled = false`
    );
    const outOfStockCount = parseInt(outOfStockResult.rows[0].count);
    console.log(`  Products with stock_quantity = 0 (should be 0 in listings): ${outOfStockCount}`);
    
    if (outOfStockCount === 0) {
      console.log('  ✓ All available products have stock\n');
    } else {
      console.log('  ⚠ Some out-of-stock products exist\n');
    }
    ...
    const unavailableResult = await pool.query(
      `SELECT COUNT(*) as count
       FROM products
       WHERE is_available = false AND status = 'approved' AND is_admin_disabled = false AND stock_quantity > 0`
    );
    ...
    // Test 5: Count products that SHOULD appear in listings
    console.log('TEST 5: Count valid available products');
    ...
    console.log(`  Valid available products: ${validCount}\n`);

    // Test 6: Verify pre-order products are filtered separately
    console.log('TEST 6: Verify pre-order filtering');
    ...
    if (nonApprovedCount === 0 && disabledCount === 0 && outOfStockCount === 0 && unavailableCount === 0) {
      ...
      console.log('  - Out-of-stock products are excluded');
