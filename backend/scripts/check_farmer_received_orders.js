...
async function checkFarmerReceivedOrders() {
  try {
    const email = 'dhelhilis@gmail.com';
    console.log(`Checking orders received by farmer: ${email}\n`);
    ...
    console.log(`Total Orders Received: ${ordersResult.rows.length}\n`);
    ...
    for (const [productName, group] of Object.entries(productGroups)) {
      ...
      for (const order of group.orders) {
        console.log(`  Order #${order.order_id}: Qty ${order.quantity} | ₱${order.total_amount} | ${order.status} | Customer: ${order.customer_name} (${order.customer_email}) | ${order.created_at}`);
      }
      console.log('---\n');
    }
    ...
    if (productGroups['Ampalaya']) {
      ...
      if (pendingOrders.length > 0) {
        ...
        for (const order of pendingOrders) {
          console.log(`  Order #${order.order_id}: Qty ${order.quantity} | Customer: ${order.customer_name} (${order.customer_email}) | ${order.created_at}`);
        }
      }
    }
