...
/**
 * Apply unified layout to Excel worksheet
 * Standardizes logo placement, title, spacing, and footer across all exports
 * 
 * @param {Object} workbook - ExcelJS workbook
 * @param {Object} ws - ExcelJS worksheet
 * @param {string} reportTitle - Title of the report
 * @param {number} colCount - Number of columns in the worksheet
 * @returns {Object} - Object containing current row number and logo height
 */
function applyUnifiedLayout(workbook, ws, reportTitle, colCount) {
  // Add logo and get dimensions
  ...
  // Add standard spacing rows
  let row = 0;
  row = ws.addRow([]).number; // Row 1: spacer with logo (already added)
  row = ws.addRow([]).number; // Row 2: spacer after logo
  row = ws.addRow([reportTitle]).number; // Row 3: title
  row = ws.addRow([]).number; // Row 4: spacer after title
  ...
  return { currentRow: row, logoHeight: targetLogoHeight };
}

/**
 * Add unified footer to Excel worksheet
 * Places footer at the bottom with standard text and center alignment
 * 
 * @param {Object} ws - ExcelJS worksheet
 * @param {number} colCount - Number of columns in the worksheet
 * @returns {number} - The row number where footer starts
 */
...
async function buildAdminOrdersExcel(pool, options, user) {
  ...
  const { currentRow } = applyUnifiedLayout(workbook, ws, 'Admin Orders Report', COL_COUNT);
  let row = currentRow;
...
async function buildFarmerOrdersExcel(pool, farmerId, options, user) {
  ...
  // Apply unified layout (logo, title, spacing)
  const { currentRow } = applyUnifiedLayout(workbook, ws, 'Farmer Orders Report', COL_COUNT);
  let row = currentRow;
...
async function buildAdminUsersExcel(pool, options, user) {
  ...
  // Apply unified layout (logo, title, spacing)
  const { currentRow } = applyUnifiedLayout(workbook, ws, reportTitle, COL_COUNT);
  let row = currentRow;
