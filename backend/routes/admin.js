...
const { getAdminOrders, buildAdminOrdersExcel, getAdminUsers, buildAdminUsersExcel, applyUnifiedLayout, addUnifiedFooter } = require('../services/orderExportService');
...
router.get('/dashboard/export.xlsx', requireAdmin, async (req, res) => {
  try {
    ...
    const COL_COUNT = 5; // Metric, Value, Change columns
    const { currentRow } = applyUnifiedLayout(wb, ws, 'Admin Dashboard Report', COL_COUNT);
    let row = currentRow;
    ...
    row = ws.addRow(['Report Period:', periodLabel]).number;
    ...
    row = ws.addRow([]).number;
    const kpiHeaderRow = ws.addRow(['Metric', 'Value', 'Change vs Previous Period']).number;
    ws.addRow(['Sold', stats.sales, `${stats.salesChange}%`]).number;
    ws.addRow(['Total Sales', `₱${stats.revenue.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, `${stats.revenueChange}%`]).number;
    ws.addRow(['Customers', stats.customers, `${stats.customersChange}%`]).number;
    ws.addRow(['Farmers', stats.farmers, `${stats.farmersChange}%`]).number;
    ws.addRow(['Harvest Attention', stats.harvest_attention, `${stats.harvest_attentionChange}%`]).number;
    row = ws.addRow([]).number;
    row = ws.addRow([]).number;
    ...
    const kpiDataStartRow = kpiHeaderRow + 1;
    const kpiDataEndRow = kpiHeaderRow + 5;
    for (let r = kpiDataStartRow; r <= kpiDataEndRow; r++) {
      for (let col = 1; col <= 3; col++) {
    ...
    row = ws.addRow(['Sold & Total Sales Trend']).number;
    const trendHeaderCell = ws.getCell(`A${row}`);
    trendHeaderCell.font = { bold: true, size: 14, color: { argb: '2E7D32' } };
    trendHeaderCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'E8F5E9' } };
    trendHeaderCell.alignment = { horizontal: 'center' };
    ws.mergeCells(`A${row}:E${row}`);

    const trendHeaderRow = ws.addRow(['Period', 'Sold', 'Total Sales']).number;
    let trendDataRow = trendHeaderRow + 1;
    for (const r of reportData.rows) {
      ws.addRow([r.period_label, r.sales, `₱${parseFloat(r.revenue).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`]).number;
      trendDataRow++;
    }

    // Style trend table
    for (let col = 1; col <= 3; col++) {
      const cell = ws.getCell(trendHeaderRow, col);
      cell.font = { bold: true, size: 12, color: { argb: 'FFFFFF' } };
      cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: '2E7D32' } };
      cell.alignment = { horizontal: 'center', vertical: 'middle' };
      cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
    }
    for (let r = trendHeaderRow + 1; r <= trendDataRow - 1; r++) {
      for (let col = 1; col <= 3; col++) {
        const cell = ws.getCell(r, col);
        cell.font = { size: 11 };
    ...
    row = trendDataRow + 1;
    ws.addRow([]).number;
    row = ws.addRow(['Top Selling Products']).number;
    ...
    const topProdHeaderRow = ws.addRow(['Product', 'Orders', 'Sold Qty', 'Total Sales']).number;
    let topProdDataRow = topProdHeaderRow + 1;
    for (const r of topProductsData.rows) {
      ws.addRow([r.product_name, r.order_count, r.sold_qty, `₱${parseFloat(r.revenue).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`]).number;
      topProdDataRow++;
    }
    ...
    ws.addRow([]).number;
    row = ws.addRow(['Top Farmers']).number;
    const topFarmHeaderCell = ws.getCell(`A${row}`);
    topFarmHeaderCell.font = { bold: true, size: 14, color: { argb: '2E7D32' } };
    topFarmHeaderCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'E8F5E9' } };
    topFarmHeaderCell.alignment = { horizontal: 'center' };
    ws.mergeCells(`A${row}:E${row}`);

    const topFarmHeaderRow = ws.addRow(['Farmer', 'Shop', 'Products', 'Orders', 'Total Sales']).number;
    let topFarmDataRow = topFarmHeaderRow + 1;
    for (const r of topFarmersData.rows) {
      ws.addRow([r.farmer_name, r.shop_name, r.product_count, r.order_count, `₱${parseFloat(r.revenue).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`]).number;
      topFarmDataRow++;
    }

    // Style top farmers table
    for (let col = 1; col <= 5; col++) {
      const cell = ws.getCell(topFarmHeaderRow, col);
      cell.font = { bold: true, size: 12, color: { argb: 'FFFFFF' } };
      cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: '2E7D32' } };
      cell.alignment = { horizontal: 'center', vertical: 'middle' };
      cell.border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
    }
    for (let r = topFarmHeaderRow + 1; r <= topFarmDataRow - 1; r++) {
      for (let col = 1; col <= 5; col++) {
        const cell = ws.getCell(r, col);
        cell.font = { size: 11 };
        cell.alignment = col === 1 ? 'left' : 'center';
