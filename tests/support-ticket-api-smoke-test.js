...
async function testSupportTicketAPI() {
  console.log('=== Support Ticket API Smoke Test ===\n');
  ...
  try {
    ...
    const adminResult = await pool.query(
      "SELECT id, username FROM users WHERE role IN ('admin', 'super_admin') LIMIT 1"
    );
    ...
    const createResponse = await fetch('http://localhost:3000/api/support-tickets', {
      ...
      headers: {
        ...
        'Authorization': `Bearer ${process.env.TEST_FARMER_TOKEN || 'test-token'}`
      },
      body: JSON.stringify({
    ...
    if (createResponse.ok) {
      ...
      console.log(`  Priority: ${data.ticket.priority}`);
    } else {
    ...
    if (farmerTicketsResponse.ok) {
      const data = await farmerTicketsResponse.json();
    ...
    console.log('\n--- Test 4: Get Ticket Details ---');
    if (farmerTicketsResponse.ok) {
      const farmerData = await farmerTicketsResponse.json();
