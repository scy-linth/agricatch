...
async function runChecks(page) {
  return page.evaluate(() => {
    ...
    all.forEach((el) => {
      try {
        ...
        if (width > winWidth + 2 && tag !== 'HTML' && tag !== 'BODY') {
          results.overflowingElements.push({ tag, class: (el.className || '').substring(0, 120), id: (el.id || '').substring(0, 60), width: Math.round(width), right: Math.round(right), text: (el.textContent || '').replace(/\s+/g, ' ').substring(0, 60) });
        }
        ...
        if ((tag === 'INPUT' || tag === 'TEXTAREA' || tag === 'SELECT') && (right > winWidth + 2 || width > winWidth + 2)) {
          results.inputsOverflowing.push({ tag, id: el.id, class: (el.className || '').substring(0, 120), width: Math.round(width), right: Math.round(right) });
        }
        if (tag === 'TABLE') {
          const tw = el.scrollWidth;
          if (tw > winWidth) results.tablesHorizontalScroll.push({ class: (el.className || '').substring(0, 120), id: el.id, width: tw, viewport: winWidth });
        }
        if (el.classList.contains('modal') || el.classList.contains('product-details-modal') || el.classList.contains('admin-detail-panel') || el.classList.contains('admin-modal') || (el.id && /modal/i.test(el.id))) {
          if (width > winWidth + 2 || rect.right > winWidth + 2 || rect.left < -2) {
            results.modalsExceedingScreen.push({ class: (el.className || '').substring(0, 120), id: el.id, width: Math.round(width), right: Math.round(rect.right), left: Math.round(rect.left) });
          }
        }
        if (el.classList.contains('sidebar') || el.classList.contains('admin-sidebar') || ['admin-sidebar','farmer-sidebar','customer-sidebar'].includes(el.id)) {
          if (width > winWidth + 2 || rect.right > winWidth + 2 || rect.left < -2) {
            results.sidebarIssues.push({ class: (el.className || '').substring(0, 120), id: el.id, width: Math.round(width), right: Math.round(rect.right), left: Math.round(rect.left), open: el.classList.contains('open') || el.classList.contains('active') });
          }
        }
        if (tag === 'HEADER' || el.classList.contains('header') || el.classList.contains('topbar') || el.id === 'header' || /topbar/i.test(el.className)) {
          if (width > winWidth + 2 || rect.right > winWidth + 2) {
            results.navbarIssues.push({ tag, class: (el.className || '').substring(0, 120), id: el.id, width: Math.round(width), right: Math.round(rect.right) });
          }
        }
        if (style.whiteSpace === 'nowrap' && width > winWidth + 2) {
          results.overflowingText.push({ tag, class: (el.className || '').substring(0, 120), text: (el.textContent || '').replace(/\s+/g, ' ').substring(0, 80), width: Math.round(width) });
        }
      } catch (e) {}
    });
    ...
    document.querySelectorAll('.float-cart-btn, .admin-float-chat-btn').forEach((btn) => {
      ...
      if (rect.width > 0) results.floatingButtonsOverlap.push({ class: (btn.className || '').substring(0, 120), right: Math.round(rect.right), bottom: Math.round(rect.bottom), width: Math.round(rect.width) });
    });
    ...
    document.querySelectorAll('.dropdown-menu, .notifications-dropdown, .user-dropdown-menu, #customer-chat-dropdown, #customer-notif-dropdown').forEach((dd) => {
      ...
      if (rect.width > 0 && (rect.right > winWidth + 2 || rect.bottom > winHeight + 2)) {
        results.clippedDropdowns.push({ class: (dd.className || '').substring(0, 120), right: Math.round(rect.right), bottom: Math.round(rect.bottom), width: Math.round(rect.width) });
      }
    });
...
(async () => {
  ...
  for (const pageInfo of TARGETS) {
    console.log(`Re-testing ${pageInfo.name}...`);
    ...
    for (const vp of VIEWPORTS) {
      ...
      // Hide loading overlays so the underlying page shell is visible for layout validation
      await page.evaluate(() => {
        ['admin-loading-screen','loading-screen','farmer-loading-screen','customer-loading-screen'].forEach(id => {
          const el = document.getElementById(id);
          if (el) el.style.display = 'none';
        });
        document.querySelectorAll('.loading-screen, .admin-loading-screen, .spinner-border').forEach(el => {
